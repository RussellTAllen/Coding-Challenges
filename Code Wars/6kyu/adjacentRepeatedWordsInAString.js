...
function countAdjacentPairs(str) {
    ...
    for (let i = 0; i < str.length-1; i++){
        if (str[i].toLowerCase() === match) continue
        else match = ''
        if (str[i].toLowerCase() === str[i+1].toLowerCase()){
            match = str[i].toLowerCase()
            count++
            i++
        }
    }
...
console.log(countAdjacentPairs('hash hasH statICMethoD staticmethOd StaTiCmetHod stAtIcMetHod Set BYteARRay dIR GlobAlS aNy slicE staticmethOd staTicmeTHod STATIcMeThoD poW'))
// 3
