...
function travel(r, zipcode){
    ...
    r = r.split(',')
    let addresses = r.filter(a => a.includes(zipcode))
    let streetNums = addresses.map(a => a.slice(0, a.indexOf(' '))).join(',')
    let streets = addresses.map(a => a.slice(a.indexOf(' ')+1, a.length - zipcode.length - 1))
    return zipcode+':'+streets+'/'+streetNums
}
