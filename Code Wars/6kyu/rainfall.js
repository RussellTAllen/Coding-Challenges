...
function findTownData(town, str){
    let arr = str.split('\n')
    for (let i = 0, len = arr.length; i < len; i++){
        if (arr[i].slice(0, arr[i].indexOf(':')) === town) {
            return arr[i].split(' ')
        }
    }
}
...
function mean(town, str){
    let data = findTownData(town, str)
    let result = 0
    if (!data) return -1
    for (let i = 1, len = data.length; i < len; i++){
        if (i !== 12) result += Number(data[i].slice(0, data[i].indexOf(',')))
        else result += Number(data[i])
    }
    return result/12
}
...
function variance(town, str){
    let data = findTownData(town, str)
    let avg = mean(town, str)
    let result = 0
    if (!data) return -1
    for (let i = 1, len = data.length; i < len; i++){
        if (i !== 12) {
            result += (Number(data[i].slice(0, data[i].indexOf(','))) - avg)**2
        } else result += (Number(data[i]) - avg)**2
    }
    return result/12
}
