...
// This works, but is extremely time inefficient
function nbaCup(resultSheet, team) {
    if (!teams.includes(team+',')) return `${team}:This team didn't play!`
    let totalScored = 0
    let gameScored = ''
    let gameConceded = 0
    let totalConceded = 0
    ...
    let draws = 0
    let points = 0
    while (resultSheet.includes(team)){
        gameScored = resultSheet.slice(resultSheet.indexOf(team) + team.length+1, resultSheet.indexOf(team) + team.length+4)
        if (gameScored.includes(',')) gameScored = gameScored.slice(0, gameScored.indexOf(','))
        totalScored += Number(gameScored)
        if (resultSheet[resultSheet.indexOf(team)-1] === ','){
            gameConceded = Number(resultSheet.slice(resultSheet.indexOf(',', resultSheet.indexOf(team)) - 3, resultSheet.indexOf(',', resultSheet.indexOf(team))))
        }else{
            gameConceded = Number(resultSheet.slice(resultSheet.indexOf(team)-4, resultSheet.indexOf(team)-1))
        }
        totalConceded += gameConceded
        if (gameScored > gameConceded){
            wins++
            points += 3
        }else if (gameScored < gameConceded) losses++
        else{ 
            draws++
            points++
        }
        resultSheet = resultSheet.slice(resultSheet.indexOf(',', resultSheet.indexOf(team)))
    }
    return `${team}:W=${wins};D=${draws};L=${losses};Scored=${totalScored};Conceded=${totalConceded};Points=${points}`
}
...
console.log(nbaCup(r, "Charlotte Hornets"))
...
// console.log(nbaCup(r, "Boston Celt"))
